...
Now find the number of principal components that capture **at least 80%** of variation and project the document term matrix (DTM) onto those components. Use the custom function `projection_fn(.dtm, .prop)` .

```{r}
# find projections based on training data
proj_out <- projection_fn(.dtm = train_dtm, .prop = 0.7) # this code will capture 70%
