...
# load(here("data", "class-summaries.RData"))
...
```{r}
kable(tbl1,
      col.names = c("Class Standing", "Count", "Percent"))
```
...
```{r}
kable(tbl2,
      col.names = c("Gender Identity", "Count", "Percent"))
```
...
```{r}
kable(tbl3,
      col.names = c("Race / Ethnicity", "Count", "Percent"))
```
...
background %>%
  ...
  # Normalize text & collapse longer phrases into beg/int/adv
  mutate(across(
    everything(),
    ~ {
      x <- str_to_lower(str_trim(.x))
      case_when(
        str_detect(x, "adv") ~ "adv",
        str_detect(x, "inter") ~ "int",
        str_detect(x, "beg|little") ~ "beg",
        TRUE ~ NA_character_
      )
    }
  )) %>%
...
background %>%
  select(contains(".prof")) %>%
  # Normalize text and classify correctly
  mutate(across(
    everything(),
    ~ {
      ...
      case_when(
        str_detect(x, "adv") ~ "adv",
        str_detect(x, "int|intermediate") ~ "int",
        str_detect(x, "beg|little") ~ "beg",
        TRUE ~ NA_character_
      )
    }
  ...
  rename_with(~ gsub("\\.prof", "", .x)) %>%
  pivot_longer(everything(), names_to = "skill", values_to = "level") %>%
  count(level, skill, name = "n") %>%
  filter(!is.na(level)) %>%  # drop unmatched NA rows
  mutate(level = factor(level, levels = c("beg", "int", "adv"))) %>%
  complete(level, skill, fill = list(n = 0)) %>%
  arrange(level) %>%
  pivot_wider(names_from = skill, values_from = n, values_fill = 0) %>%
  kable()
```
...
-   Cluster 2: intermediate, strong math/stat background

-   Cluster 3: intermediate, weaker programming comfort
:::
