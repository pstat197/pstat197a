...
time_ssvd <- end - start; time_ssvd

start <- Sys.time()
prcomp_out <- prcomp(claims_dtm, center = T)
end <- Sys.time()
time_prcomp <- end - start; time_prcomp
...
# partition data
set.seed(102925)
partitions <- claims %>% initial_split(prop = 0.8) # proportion can vary
# this means 80% of data will be used for training, 20% for testing
...
proj_out <- projection_fn(.dtm = train_dtm, .prop = 0.8) # change to 0.8
